...
- 合并上游 master（5c39e71）：串口控制台页面按 picocom 重译，删除 4 张 minicom 截图；WSL 设置、E3 eMMC 烧录页同步；`|picocom|` 替换项与代码块字体修复同步到中文树。
- 首页增加“以英文官方文档为准”的提示（维护者在 issue #98 中的要求），结构检查白名单相应放行该 URL。
- 新增 `translations/zh_CN/tools/`：汉化原理与流程说明（README）、迁移后的检查/复用脚本（路径改为仓库布局）、`sync_status.py`（按 PROGRESS.tsv 找出上游已变更页面并导出 diff）、三份 AI 提示词。
- 新增图片汉化工具链 `tools/images/`（Vision OCR → 词表匹配 → Pillow 原位替换），首批处理 22 张示意图：连接示意图 4 张、循环/Python 缓冲区、多板连接 3 张、Deep Memory、SCPI 流程图 2 张、DB9 引脚图、测量装置图、网线贴纸标注 2 张、跳线注意事项 2 张、Web 应用架构图 4 张。
- 检查脚本在新布局下运行：翻译检查 378/378、结构差异 0、本地严格 Sphinx 构建通过。

## 2026-09-18 — 批次 071

- 图片汉化第二批 26 张：板型识别指南标注照片 3 张、带宽/平坦度/输出阻抗/插值曲线 8 张、跳线设置图 2 张、跳线实物照片说明框 2 张、阻抗分析仪测量范围、ST-Link 排线布局、FPGA 目录结构 2 张、逻辑分析仪接线 2 张、阻抗分析仪 R_shunt 接线 4 张（同图）、网络连接类型总览、SDR 模块框图、I2C 单次加速器原理图。
- 工具改进：遮罩与文字分两遍绘制避免相邻行互相擦除；窄高框自动竖排；`#`/`-` 开头的注释左对齐；背景色取框内精确主色；OCR 改用 ImageIO 读取原始像素，修正带旋转元数据 PNG 的坐标；低分辨率图放大后识别。
- 13 张 FPGA 模块 ASCII 框图保留英文（等宽排版，按代码处理）。本批无需生图模型重绘。
